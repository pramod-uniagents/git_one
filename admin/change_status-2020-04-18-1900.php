...
if ($requestMethod == 'post' && $_POST['change_status'] == 'Y' && decrypt($_REQUEST['agentId']) > 0) {
    ...
    if ($agentId > 0) {
        ...
        if ($_POST['unica_application_enabled']) {
            ...
            $unica_application_enabled = $_POST['unica_application_enabled'];
        }
        ...
        $data = array(
            ...
            'crm_uses_type' => $_POST['crm_uses_type'],
            'mobile_app_owner' => $_POST['mobile_app_owner'],
            'app_is_match_title' => $app_is_match_title,
            'app_is_featured_title' => $app_is_featured_title,
            'unica_application_enabled' => $unica_application_enabled,
                ...
                foreach ($crm_uses_types_array as $key => $crm_uses_type_name) { ?>
                    ...
                    <label class="radiobutton">
                        <input type="radio" name="crm_uses_type" required
                               value="<?php echo $key; ?>" <?php echo $agent_details['crm_uses_type'] == $key ? 'checked' : ''; ?>>
